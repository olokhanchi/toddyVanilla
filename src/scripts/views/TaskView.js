...
export default class TaskView {
  ...
  renderDefaultTasks(type, tasks) {
    ...
    for (const task of tasks) {
      taskItemsArray.push(
        U.taskTemplate(task.id, task.description, this.defaultTaskTitle)
      );
    }
  ...
  showTaskField() {
    this.currentClickedContent.insertAdjacentHTML(
      "afterbegin",
      this.taskTemplate
    );
    this.currentTaskField = this.currentClickedContent.querySelector(
